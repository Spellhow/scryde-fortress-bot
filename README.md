...
- `TG_CHAT_DEBUG`
- `GEMINI_API_KEY`
- optional: `GEMINI_MODEL` (default `gemini-2.5-flash-lite`; only the hardcoded Free-Tier allowlist is accepted)
...
- Gemini calls are fail-closed to stable Free-Tier model IDs only: `gemini-2.5-flash-lite` (primary) and `gemini-3.1-flash-lite` (Wiki fallback); any other `GEMINI_MODEL` value is ignored/blocked
